        ...
        for j in range(len(tab[i])):
            if tab[i][j] == CellState.WALL:
                grille.create_rectangle(
                    j * 25, i * 25, (j + 1) * 25, (i + 1) * 25, fill="red"
                )
            if (i, j) in good_moves:
                grille.create_rectangle(
                    j * 25, i * 25, (j + 1) * 25, (i + 1) * 25, fill="grey"
                )
            elif tab[i][j] == CellState.EXIT:
                grille.create_rectangle(
                    j * 25, i * 25, (j + 1) * 25, (i + 1) * 25, fill="orange"
                )
...
def on_key(
    event, robot: Robot, grille: Canvas, maze: Maze, fenetre: Tk, good_moves: list
):
    key = event.keysym
