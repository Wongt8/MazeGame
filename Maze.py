...
class Maze:
    ...
    def build_maze(self, walls):
        while walls:
            ...
            if rand_wall[1] != 0:
                if (
                    self.maze[rand_wall[0]][rand_wall[1] - 1] == CellState.UNVISITED
                    and self.maze[rand_wall[0]][rand_wall[1] + 1] == CellState.EMPTY
                ):
            ...
            if rand_wall[0] != 0:
                if (
                    self.maze[rand_wall[0] - 1][rand_wall[1]] == CellState.UNVISITED
                    and self.maze[rand_wall[0] + 1][rand_wall[1]] == CellState.EMPTY
                ):
                    s_cells = self.surroundingCells(rand_wall)
                    if s_cells < 2:
                        self.maze[rand_wall[0]][rand_wall[1]] = CellState.EMPTY
    ...
    def transform_to_matrix(self):
        ...
        for line in range(len(self.maze)):
            matrix.append([])
            for i in range(len(self.maze[line])):
                if self.maze[line][i] != CellState.WALL:
